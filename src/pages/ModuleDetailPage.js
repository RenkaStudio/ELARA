        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-white p-4 rounded-lg border border-border-color">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-gray-500 text-xs">Kesulitan</p>
                  <p className="text-lg font-bold text-gray-800">{module.difficulty || 'Tidak Diketahui'}</p>
                </div>
              </div>
            </div>
            <div className="bg-white p-4 rounded-lg border border-border-color">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-gray-500 text-xs">Waktu Estimasi</p>
                  <p className="text-lg font-bold text-gray-800">{module.estimatedTime || 'Tidak Diketahui'}</p>
                </div>
              </div>
            </div>
            <div className="bg-white p-4 rounded-lg border border-border-color">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-gray-500 text-xs">Gaya Belajar</p>
                  <p className="text-lg font-bold text-gray-800">{learningProfile.learningStyle || 'Tidak Diketahui'}</p>
                </div>
              </div>
            </div>
            <div className="bg-white p-4 rounded-lg border border-border-color">
          ...
          <div className="bg-bg-card rounded-xl shadow-soft border border-border-color p-6 mb-6">
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-xl font-bold text-text-dark">Ringkasan AI Disesuaikan</h3>
            </div>
            <div className="prose max-w-none mb-4">
              <p className="text-text-dark leading-relaxed">
                {module.learningStyleSummary || module.summary || 'Belum ada ringkasan yang dibuat. Upload modul lagi untuk menghasilkan ringkasan.'}
              </p>
            </div>
            ...
            {module.keyTopics && module.keyTopics.length > 0 && (
              <div className="mt-4 pt-4 border-t border-border-color">
                ...
                <div className="flex flex-wrap gap-2">
                  {module.keyTopics.slice(0, 10).map((topic, index) => (
                    <span 
                      key={index} 
            ...
            {(module.difficulty || module.estimatedTime) && (
              <div className="mt-4 pt-4 border-t border-border-color">
                <h4 className="font-semibold text-text-dark mb-3">Statistik Modul</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {module.difficulty && (
                    <div className="flex items-center p-3 bg-gray-50 rounded-lg">
                      <BarChart3 className="text-yellow-600 mr-2" size={16} />
                      <div>
                        <p className="text-xs text-gray-500">Tingkat Kesulitan</p>
                        <p className="font-medium text-gray-800 capitalize">{module.difficulty}</p>
                      </div>
                    </div>
                  )}
                  {module.estimatedTime && (
                    <div className="flex items-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-800">{module.estimatedTime}</p>
                      </div>
                    </div>
                  )}
                </div>
