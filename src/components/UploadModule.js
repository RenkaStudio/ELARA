  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!quizQuestions || quizQuestions.length === 0) {
        throw new Error('Tidak dapat membuat kuis dari file. Konten mungkin terlalu sedikit atau tidak sesuai.');
      }
      ...
      const moduleInfo = {
        ...
        ...summaryData // Spread all summary properties into module info
      };
      ...
      if (error.message.includes('timeout')) {
        ...
        // Even if API fails, we can still process with local methods
        setUploadStatus('error');
        setProgress(0);
        console.warn('API error, but system should fallback to local processing. Error:', error.message);
      } else {
        setUploadStatus('error');
        setProgress(0);
      }
